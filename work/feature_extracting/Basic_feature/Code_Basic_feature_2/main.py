...
def run():
    ...
    with Pool(8) as pool:
        ret = pool.map(run_train_feature, [i for i in range(workers_train)])
    print(' train done: %d' % np.sum(ret))

    with Pool(8) as pool:
        pool.map(run_test_feature, [i for i in range(workers_test)])
    print(' test done: %d' % np.sum(ret))
